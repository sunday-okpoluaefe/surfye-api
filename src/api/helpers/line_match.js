...
module.exports = line_match;
module.exports.default = line_match;

function line_match(lines1, lines2, threshold) {

    const segments1 = linesToSegments(lines1);
    ...
    for (let i = 0; i < segments2.length; i += 4) {
        index.add(
            Math.min(segments2[i], segments2[i + 2]),
            Math.min(segments2[i + 1], segments2[i + 3]),
            Math.max(segments2[i], segments2[i + 2]),
    ...
    while (segments1.length) {
        const by = segments1.pop();
        const bx = segments1.pop();
        const ay = segments1.pop();
        const ax = segments1.pop();
    ...
    return diff.length > 0 ? diff[0] : []//linesToSegments(diff);
}
